...
export default function MissionCard({ mission, isCompleted, onComplete }: Props) {
  return (
    <button
      ...
      className={`w-full flex items-center gap-3 px-4 py-3 rounded-2xl text-left transition-all duration-300 transform active:scale-[0.98] ${isCompleted
    ...
    >
      ...
      <div
        className={`w-6 h-6 rounded-full flex items-center justify-center border-2 transition-all duration-300 flex-shrink-0 ${isCompleted
            ? 'border-emerald-500 bg-emerald-500 scale-110 shadow-[0_0_10px_rgba(16,185,129,0.3)]'
            : 'border-gray-300 dark:border-gray-600'
          }`}
      >
        {isCompleted && <span className="text-white text-xs font-bold animate-fade-in-up">✓</span>}
      </div>
    </button>
  );
}
